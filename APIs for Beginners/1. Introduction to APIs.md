...
## Authorization (Auth):
APIs user authorization (auth) to ensure that clients' requests access data securely. This can involve authenticating the sender of the request and ensuring they have the permission to access or manipulate relevant data. Three commonly used methods are:
1. `Basic Auth`: requires sending a verified username and password.
2. `Oauth`: requires sending a granted access token with the request, and it is useful when multiple application needs access.
3. `API Key`: requires sending a secrey key-value pair in the request header or query parameters.

### Notes:
* If you are integrating a third party API, the required authentication will be specified by the API documentation with exact format.
* The authentication data can be added in the `Headers` or the `Authorization` tabs.
* We can specify the data type we want to recieve in a response in the authentication process.
