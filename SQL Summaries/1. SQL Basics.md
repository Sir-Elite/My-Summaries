...
4. `AND` & `BETWEEN` These allow you to combine operations where all combined conditions must be true.
   ...
   `WHERE column >= 6 AND column <= 10` is equivalent to `WHERE column BETWEEN 6 AND 10`.

   `WHERE occurred_at >= '2016-04-01' AND occurred_at <= '2016-10-01'` is equivalent to `WHERE occurred_at BETWEEN '2016-04-01' AND '2016-10-01'`
